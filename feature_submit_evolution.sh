...
seed=$1
config_file=${2:-config_feature.yaml}

if [ -z "$seed" ]; then
    echo "ERROR: Seed is required as first argument"
    echo "Usage: sbatch feature_submit_evolution.sh <seed> [config_file]"
    echo "Example: sbatch feature_submit_evolution.sh 42 config_feature.yaml"
    exit 1
fi
